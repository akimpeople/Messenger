...
class LoginViewController: UIViewController {
    ...
    private let imageView: UIImageView = {
        ...
        imageView.image = UIImage(named: "peopleSquare")
    ...
    override func viewDidLoad() {
        ...
        signinObserver = NotificationCenter.default.addObserver(forName: .didSignInNotification,
                                                                object: nil,
                                                                queue: .main,
        ...
        FirebaseAuth.Auth.auth().signIn(withEmail: email, password: password, completion: { [weak self] authResult, error in
            ...
            DatabaseManager.shared.getDataFor(path: safeEmail, completion: { result in
                ...
                case .failure(let error):
                    print("Failed to red data with error: \(error)")
                }
            })
...
extension LoginViewController: LoginButtonDelegate {
    ...
    func loginButton(_ loginButton: FBLoginButton, didCompleteWith result: LoginManagerLoginResult?, error: Error?) {
        guard let token = result?.token?.tokenString else {
            print("Failed sign In with facebook")
            return
        }
        ...
        facebookRequest.start(completionHandler: { _, result, error in
            ...
            FirebaseAuth.Auth.auth().signIn(with: credential, completion: { [weak self] authResult, error in
                ...
                guard authResult != nil, error == nil else {
                    if let error = error {
                        print("Facebook credential signin failed, MFA may be needed - \(error)")
                    }
                    return
                }
